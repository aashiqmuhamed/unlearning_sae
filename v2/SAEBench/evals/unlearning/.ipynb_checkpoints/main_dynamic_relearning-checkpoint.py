...
def run_eval(
    ...
    finetuned = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.bfloat16, device_map='cpu')

    model = HookedTransformer.from_pretrained_no_processing(model_name=config.model_name, device=device,  dtype=config.llm_dtype, hf_model=finetuned, cache_dir="/data/datasets/wmdp_test/model_dir")
...
def arg_parser():
    ...
    parser.add_argument("--artifacts_path", type=str, required=True, help="Path to directory to save artifacts")

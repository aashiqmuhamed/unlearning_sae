        ...
        with tqdm(total=num_batches) as pbar:
            for idx in range(num_batches):
                ...
                if method == "rmu":
                    ...
                    # Gradient descent: Use NLL loss
                    # unlearn_loss = torch.tensor([0]) #-compute_nll_loss(updated_model, unlearn_inputs)
                ...
                loss = (unlearn_loss )/ args.gradient_accumulation_steps
                # loss = ( retain_loss)/ args.gradient_accumulation_steps
                loss.backward()
            ...
            epoch_path = os.path.join(args.output_dir, f"epoch_{epoch}")
            os.makedirs(epoch_path, exist_ok=True)
            updated_model.save_pretrained(epoch_path)
            tokenizer.save_pretrained(epoch_path)
            print(f"\nSaved model after epoch {epoch} to {epoch_path}")

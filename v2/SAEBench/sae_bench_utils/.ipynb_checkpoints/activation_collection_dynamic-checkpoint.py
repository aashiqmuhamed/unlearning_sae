...
@jaxtyped(typechecker=beartype)
@torch.no_grad
def get_feature_activation_sparsity_old(
    tokens: Int[torch.Tensor, "dataset_size seq_len"],
    model: HookedTransformer,
    sae: SAE | Any,
    batch_size: int,
...
# def get_feature_activation_sparsity(
#     tokens: Int[torch.Tensor, "dataset_size seq_len"],
#     model,  # type: HookedTransformer (or similar)
#     sae,    # type: SAE or similar object having attributes: device, W_dec, and encode()
#     batch_size: int,
#     layer: int,
#     hook_name: str,
#     mask_bos_pad_eos_tokens: bool = False,
...
#     running_squared_gradients = torch.zeros_like(sae.W_dec, dtype=dtype, device=device)
#     total_tokens = 0
...
#         resid_BLD = resid_BLD.to(dtype=dtype)
#         sae_act_BLF = sae.encode(resid_BLD)
#         rec_BLD = torch.einsum("fd,blf->bld", sae.W_dec, sae_act_BLF)
#         error_BLD = rec_BLD - resid_BLD
...
#         attn_mask_BL = attn_mask_BL.to(device=device)
#         mask_expanded = attn_mask_BL.unsqueeze(-1).to(dtype=dtype)
        
#         grad_batch = torch.einsum(
#             "blf,bld->fd",
#             (2 * sae_act_BLF * mask_expanded).to(dtype=dtype),
#             error_BLD.to(dtype=dtype)
#         )
        
#         running_squared_gradients += grad_batch.pow(2)  # Square the gradients before accumulating
#         total_tokens += attn_mask_BL.sum().item()
    
#     avg_squared_gradients = running_squared_gradients / total_tokens
#     grad_norms = torch.norm(avg_squared_gradients, dim=-1)  # Take norm of averaged squared gradients
#     return grad_norms.to(dtype=torch.float32)
